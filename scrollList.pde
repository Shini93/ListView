class scrollList { //<>//
  ...
  void drawField(int y, int r, color c, int tH) {
    if (checkFieldVisible(r, y) == true) {
      ...
      g.text(fields.get(y).get(r), pos.x + (y - 1) * hFieldX + posX + tPosX, pos.y + r * hFieldY +  posY -(hFieldY - tH) * 0.5);
    }
  }

  void drawFields() {
    g.fill(0);
    int tH = round(g.textAscent() + g.textDescent());
    for (int y = 0; y < fields.size(); y ++) {
      for (int r = 0; r < fields.get(y).size(); r++) {
        drawField(y, r, 200, tH);
      }
    }
    g.fill(125);
  }
